import { Adapter, AdapterEvents, DrawImageOptions } from '@rtjs/common';
...
export class CanvasAdapter implements Adapter {
	...
	public image(image: Image, x: number, y: number, options?: DrawImageOptions): void {
		const { scale, scaleX, scaleY, rotation } = options || {};
		...
		if (rotation) {
			...
			this.ctx.drawImage(
				image.elem,
				this._toCanvasX(x) - image.width / 2,
				this._toCanvasY(y) - image.height / 2,
				image.width * scale,
				image.height * scale
			);
		} else if (scaleX !== undefined || scaleY !== undefined) {
			this.ctx.drawImage(
				...
				this._toCanvasX(x) - image.width / 2,
				this._toCanvasY(y) - image.height / 2,
				image.width * (scaleX ?? 1),
				image.height * (scaleY ?? 1)
			);
		} else {
			this.ctx.drawImage(image.elem, this._toCanvasX(x) - image.width / 2, this._toCanvasY(y) - image.height / 2, image.width, image.height);
		}
	}
